...
const PopularCard = React.forwardRef<HTMLDivElement, PopularCardProps>(({ name, link, imageUrl, rating, iconName, className, ...props }, ref) => {
  return (
    <div className={cn('relative rounded flex flex-col justify-between w-40 h-52 p-2 overflow-hidden', className)} ref={ref} {...props}>
      <div className='absolute -top-2 left-0 -z-10 w-full h-1/2 bg-gradient-to-b from-base-black to-transparent' />
      ...
      <div className='flex flex-row justify-between'>
        ...
        {iconName && <Icon name={iconName} className='w-6 h-6 text-base-white' />}
      </div>
      ...
      <img
        ...
        alt={name}
      />
      <a className='font-primary font-bold text-base-white' href={link}>{name}</a>
    </div>
  );
});
